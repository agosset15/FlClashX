...
import com.kago.vpnapp.common.GlobalState as CommonGlobalState
import com.kago.vpnapp.common.receiveBroadcastFlow
import com.kago.vpnapp.extensions.getActionIntent
import com.kago.vpnapp.plugins.AppPlugin
import com.kago.vpnapp.plugins.TilePlugin
...
object GlobalState {
    ...
    const val NOTIFICATION_CHANNEL = "KaGo"
        ...
        CommonGlobalState.launch {
            runStateFlow.collect { state ->
                withContext(Dispatchers.Main) {
                    ...
                    if (state != RunState.PENDING) {
                        runCatching {
                            com.kago.vpnapp.services.FlClashXTileService.requestUpdate(
                        ...
                        runCatching {
                            ...
                            for (cls in arrayOf(
                                com.kago.vpnapp.widgets.OnOffWidgetProvider::class.java,
                                com.kago.vpnapp.widgets.ModeWidgetProvider::class.java,
                            )) {
                    ...
                    BroadcastAction.SERVICE_DESTROYED.action -> {
                        ...
                        CommonGlobalState.launch {
                            runLock.withLock {
                                ...
                                if (com.kago.vpnapp.common.SavedParams.isVpnActive() && recentStart) {
    ...
    suspend fun handleSyncState() {
        runLock.withLock {
            val vpnActive = com.kago.vpnapp.common.SavedParams.isVpnActive()
            if (!vpnActive) {
                runTime = 0L
                runStateFlow.tryEmit(RunState.STOP)
                return@withLock
            ...
            runCatching {
                ...
                if (state == RunState.START) {
                    if (runTime != 0L) {
                        ...
                        com.kago.vpnapp.common.SavedParams.setStartTime(runTime)
                    } else {
                        ...
                        runTime = com.kago.vpnapp.common.SavedParams.getStartTime()
                            ?: System.currentTimeMillis().also {
                                com.kago.vpnapp.common.SavedParams.setStartTime(it)
                            }
                    }
                }
    ...
    fun handleToggle() {
        CommonGlobalState.launch {
            runLock.withLock {
                ...
                val running = com.kago.vpnapp.common.SavedParams.isVpnActive() ||
    ...
    private suspend fun triggerStart() {
        ...
        val hasSavedParams = com.kago.vpnapp.common.SavedParams.loadQuickStartParams() != null
        if (!hasSavedParams) {
            runStateFlow.tryEmit(RunState.STOP)
        ...
        com.kago.vpnapp.common.SavedParams.setVpnActive(true)
        startRequestedAt = android.os.SystemClock.elapsedRealtime()
        runCatching {
            val intent = android.content.Intent(ctx, com.kago.vpnapp.service.FlVpnService::class.java)
    ...
    private suspend fun triggerStop() {
        ...
        if (runStateFlow.value == RunState.STOP &&
            !com.kago.vpnapp.common.SavedParams.isVpnActive()) return
        ...
        runCatching {
            val ctx = CommonGlobalState.application
            val stopIntent = android.content.Intent(ctx, com.kago.vpnapp.service.FlVpnService::class.java)
                .setAction(com.kago.vpnapp.service.FlVpnService.ACTION_STOP)
